...
class BasicService(object):
    ...
    def datagram_received(self, data, addr):
        ...
        """
        logger.info('VM in Central Cloud received packet from %s:', addr)
    ...
    def process_datagram(self, data, addr):
        ...
        """
        logger.info('Service was processed in Central Cloud')
        logger.debug('%s %s', addr, binascii.hexlify(data))
        ...
        if nsh_decode.is_data_message(data):
            ...
            if nsh_decode.is_vxlan_nsh_legacy_message(data):
                ...
                addr_l[1] = 6633
                addr = tuple(addr_l)
                if sfc_globals.sff_queued_packets / 100 > 0.3:
                   time.sleep(sfc_globals.sff_queued_packets/200)
                else:
                   time.sleep(sfc_globals.sff_queued_packets/100)
                self.transport.sendto(rw_data, addr)
#                self.transport.sendto('192.168.1.2', 6633)
...
class MyVMService(BasicService):
    def __init__(self, loop):
        super(MyVMService, self).__init__(loop)

        self.service_type = VM
